...
class UserSignaturesController < ApplicationController
  ...
  def update
    ...
    return redirect_to profile_path, notice: I18n.t('unable_to_save_signature') if file.blank?
    ...
    if @user_config.update(value: attachment.uuid)
      redirect_to profile_path, notice: I18n.t('signature_has_been_saved')
    else
      redirect_to profile_path, notice: I18n.t('unable_to_save_signature')
    end
  end
  ...
  def destroy
    ...
    redirect_to profile_path, notice: I18n.t('signature_has_been_removed')
  end
